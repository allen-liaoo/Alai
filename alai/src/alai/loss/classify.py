...
class BinaryCrossEntropy(Loss):
    ...
    @staticmethod
    def validateArgs(output, targets):
...
class CrossEntropySoftmax(Loss):
    ...
    # targets: (...1, C, ...2)
    @staticmethod
    def validateArgs(logits, targets, classAxis):
    ...
    @Loss.fwd_reduce
    @utils.validate(validateArgs)
    def forward(self, logits, targets, classAxis= 0):
    ...
    @Loss.bwd_reduce
    @utils.validate(validateArgs)
    def backward(self, logits, targets, classAxis= 0):
